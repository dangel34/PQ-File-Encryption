...
Ranked by security value per unit of effort (2026-07-08 ecosystem review): cheap CI and supply-chain wins first, then hardening work, then performance. Items 1 and 2 of the original ranking, plus 2-5 of an even earlier ranking (cargo-semver-checks, advisory scan, zizmor, compat vectors) and the memory-locking item, are done and moved to the completed list above.

### 1. Optional formally verified ML-KEM backend (`libcrux-ml-kem`)

Cryspen's `[libcrux-ml-kem](https://crates.io/crates/libcrux-ml-kem)` implements all three ML-KEM variants with code verified in F* (via hax) for panic freedom, correctness, and secret independence, plus AVX2 and NEON backends that outperform the pure-Rust `ml-kem` crate. A feature-gated backend (`kem-libcrux`) behind the existing typed-key API would let users opt into the verified implementation with no wire-format change. Caveat for honest documentation: independent analysis ([eprint 2026/192](https://eprint.iacr.org/2026/192.pdf)) found only ~58% of the Rust proof surface is actually SMT-checked and the NEON backend is admitted without proofs, so this is defense in depth, not a silver bullet.

**Prerequisite done (2026-07-10):** `pqfile/tests/kem_oracle.rs` is a permanent cross-implementation oracle test (`libcrux-ml-kem` as a dev-dependency only, non-wasm32) proving `ml-kem` and `libcrux-ml-kem` agree byte-for-byte on FIPS 203 for all three parameter sets: same seed derives the same public key, same `(ek, m)` produces the same deterministic ciphertext and shared secret, and each crate correctly decapsulates the other's ciphertext. The hybrid X25519+ML-KEM-768 variant needs no separate case since it reuses the plain 768 code path.

**Remaining work:** the actual `kem-libcrux` feature and production backend swap. This is a bigger lift than the oracle test - ML-KEM operations are not centralized in one module; `encrypt.rs`, `decrypt.rs`, and `keygen.rs` each construct `ml-kem` typed keys directly (`EncapsulationKey*::new`, `DecapsulationKey*::from_seed`, `.encapsulate()`/`.decapsulate()`) at multiple call sites per variant (plain, encrypted, hardware-backed key loading). A real backend switch needs a new `pqfile/src/kem_backend.rs` abstraction that both implementations satisfy, with every one of those call sites routed through it - including for the default (non-libcrux) build, since they'd move behind the same abstraction. Scoped down deliberately on 2026-07-10 to just the oracle test first, given the risk of touching every ciphertext-producing and -consuming path in the library in one pass.

### 2. Deterministic benchmark gate (`iai-callgrind`)
...
### 3. Profile-guided optimization for release binaries

`[cargo-pgo](https://github.com/Kobzol/cargo-pgo)` automates the instrument / run-workload / rebuild cycle. On top of the existing thin-LTO release profile, 5-15% is typical for hot crypto loops. The workload step (a representative encrypt/decrypt corpus) adds release-pipeline complexity, so do this after item 2 exists and the win can be measured rather than assumed.
...
Implementation notes: prefer the default setup toggle (repository Settings, Code security and analysis) over a committed workflow file; Rust scans with `build-mode: none`, so no build customization is needed. If a workflow file is used instead, it needs `security-events: write` permission and is covered automatically by the zizmor CI job.
